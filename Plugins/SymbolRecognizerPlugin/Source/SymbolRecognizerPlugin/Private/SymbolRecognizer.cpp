...
bool USymbolRecognizer::bFlipYAxis_Android = true;
...
void USymbolRecognizer::SetFlipYAxisForAndroid(bool bShouldFlip)
{
	bFlipYAxis_Android = bShouldFlip;
}
...
void USymbolRecognizer::Draw(FVector2D InDrawPosition)
{
#if PLATFORM_ANDROID
	if (bFlipYAxis_Android)
	{
		InDrawPosition.Y *= -1.0f;
	}
#endif
...
void USymbolRecognizer::SaveSRData()
{
	...
	{
		SRDataPackage = CreatePackage(*GetSRDataPackageName());
	}
