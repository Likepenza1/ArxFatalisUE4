	...
	{	
		...
		else
		{
			for (const FString& SRDPath : FoundSRDirectories)
			{
				...
				TArray<FString> FoundSymbolsDirectories;
				FindDirectories(FoundSymbolsDirectories, SRD.Path, true);
				SRD.SymbolsAmount = FoundSymbolsDirectories.Num();
				Profiles.Add(SRD);
			}
		}
	}
...
void USRToolManager::TrainNetwork()
{
	...
	//START ASYNC TASK
	 (new FAutoDeleteAsyncTask<NetworkTrainingAsyncTask>(
		 GetSymbolRecognizer()->GetNeuralNetworkRef(false),
		 TrainingSets,
		 GetCurrentProfileRef().SymbolsAmount,
		 TrainingImagesCount,
		 GetInputNodesCount(),
		 GetCurrentProfileRef().HiddenNodes,
		 GetCurrentProfileRef().LearningRate,
		 EpochValue, //0 epchs means auto training until Accuracy is reached.
		 GetCurrentProfileRef().AcceptableTrainingAccuracy,
		 GetCurrentProfileRef().DeltaTwoBestOutcomes,
		 FTrainingTaskCompleteDelegate::CreateUObject(this, &USRToolManager::OnTrainingComplete),
		 FTrainingTaskStopDelegate::CreateUObject(this, &USRToolManager::OnTrainingStop))
		 )->StartBackgroundTask();
...
void USRToolManager::OnTrainingComplete()
{
	...
	FScopeLock ScopeLock(&CriticalSecion);
...
bool USRToolManager::CanEditChange(const FProperty* InProperty) const
{
